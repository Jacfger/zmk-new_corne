...
#   Row 1: 6 left  |  left/center/right  |  6 right         = 15
...
def format_keymap(content: str) -> str:
    ...
    for row_idx in range(3):
        ...
        for ci in range(6):
            right_unified[ci] = max(
                right_unified[ci], col_widths[row_idx][right_gi][ci]
            )
    for row_idx in range(3):
        right_gi = len(col_widths[row_idx]) - 1
    ...
    max_left_w = max(section_w(ri, sec[0]) for ri, sec in enumerate(ROW_SECTIONS))
    max_mid_w = max(section_w(ri, sec[1]) for ri, sec in enumerate(ROW_SECTIONS))
    left_w = max_left_w

    # Anchor vertical joystick column to the center key of row 1's middle trio.
    row1_mid = ROW_SECTIONS[1][1]
    mid_anchor_offset = group_width(col_widths[1][row1_mid[0]]) + 2
    ...
    def format_alpha_row(row_idx: int, row_bindings: list[str]) -> str:
        ...
        # Left section
        left_parts = [
            format_group(groups[gi], col_widths[row_idx][gi]) for gi in left_gi
        ]
        left_str = "  ".join(left_parts)
        left_pad = max_left_w - len(left_str)

        # Mid section
        mid_parts = [format_group(groups[gi], col_widths[row_idx][gi]) for gi in mid_gi]
        mid_str = "  ".join(mid_parts)
        if len(mid_gi) == 1:
            mid_pad_left = mid_anchor_offset
            mid_pad_right = max_mid_w - mid_pad_left - len(mid_str)
        else:
            mid_pad_left = 0
            mid_pad_right = max_mid_w - len(mid_str)
        ...
        for i, gi in enumerate(right_gi):
            right_parts.append(
                format_group(
                    groups[gi],
                    col_widths[row_idx][gi],
                    pad_last=(i < len(right_gi) - 1),
                )
            )
...
def main():
    ...
    keymap_path = (
        Path(args[0])
        if args
        else Path(__file__).parent / "config" / "eyelash_corne.keymap"
    )
